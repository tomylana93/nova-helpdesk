<script setup lang="ts">
import { Head, Link, setLayoutProps, useForm } from '@inertiajs/vue3';
import { update } from '@/actions/App/Http/Controllers/Admin/MasterData/BranchController';
import InputError from '@/components/InputError.vue';
import PageWrapper from '@/components/PageWrapper.vue';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import {
    Select,
    SelectContent,
    SelectItem,
    SelectTrigger,
    SelectValue,
} from '@/components/ui/select';
import { Spinner } from '@/components/ui/spinner';
import { useTrans } from '@/composables/useTrans';
import { dashboard } from '@/routes';
import { index as adminMasterDataIndex } from '@/routes/admin/master-data';
import { edit, index } from '@/routes/admin/master-data/branches';
import type { SelectOption } from '@/types';

type Branch = {
    id: string;
    code: string;
    name: string;
    status: 'active' | 'inactive';
};

type Props = {
    branch: Branch;
    statusOptions: SelectOption[];
};

type EditBranchFormData = {
    code: string;
    name: string;
    status: Branch['status'];
};

const props = defineProps<Props>();

defineOptions({ inheritAttrs: false });

const { trans } = useTrans();

const form = useForm<EditBranchFormData>({
    code: props.branch.code,
    name: props.branch.name,
    status: props.branch.status,
});

function submit(): void {
    form.submit(update(props.branch.id));
}

function reset(): void {
    form.resetAndClearErrors();
}

setLayoutProps({
    breadcrumbs: [
        {
            title: 'Dashboard',
            href: dashboard(),
        },
        {
            title: trans('admin.master_data.title'),
            href: adminMasterDataIndex(),
        },
        {
            title: trans('admin.master_data.branch.index.title'),
            href: index(),
        },
        {
            title: trans('admin.master_data.branch.edit.title'),
            href: edit(props.branch.id),
        },
    ],
});
</script>

<template>
    <Head :title="trans('admin.master_data.branch.edit.title')" />

    <PageWrapper
        :title="trans('admin.master_data.branch.edit.heading')"
        :description="trans('admin.master_data.branch.edit.description')"
    >
        <form class="flex flex-col gap-6" @submit.prevent="submit">
            <div class="grid gap-2">
                <Label for="code">{{
                    trans('admin.master_data.branch.label.code')
                }}</Label>
                <Input
                    id="code"
                    name="code"
                    v-model="form.code"
                    autofocus
                    :placeholder="
                        trans('admin.master_data.branch.placeholder.code')
                    "
                />
                <InputError :message="form.errors.code" />
            </div>
            <div class="grid gap-2">
                <Label for="name">{{
                    trans('admin.master_data.branch.label.name')
                }}</Label>
                <Input
                    id="name"
                    name="name"
                    v-model="form.name"
                    :placeholder="
                        trans('admin.master_data.branch.placeholder.name')
                    "
                />
                <InputError :message="form.errors.name" />
            </div>
            <div class="grid gap-2">
                <Label for="status">{{
                    trans('admin.master_data.branch.label.status')
                }}</Label>
                <Select
                    id="status"
                    v-model="form.status"
                    name="status"
                    :aria-invalid="!!form.errors.status"
                >
                    <SelectTrigger class="w-full">
                        <SelectValue
                            :placeholder="
                                trans(
                                    'admin.master_data.branch.placeholder.status',
                                )
                            "
                        />
                    </SelectTrigger>
                    <SelectContent>
                        <SelectItem
                            v-for="option in props.statusOptions"
                            :key="option.value"
                            :value="option.value"
                        >
                            {{ option.label }}
                        </SelectItem>
                    </SelectContent>
                </Select>
                <InputError :message="form.errors.status" />
            </div>
            <div
                class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center"
            >
                <Button
                    type="submit"
                    :disabled="form.processing"
                    class="w-full sm:w-auto"
                >
                    <Spinner v-if="form.processing" />
                    {{ trans('admin.master_data.branch.action.update') }}
                </Button>
                <Button
                    type="button"
                    variant="secondary"
                    :disabled="form.processing"
                    class="w-full sm:w-auto"
                    @click="reset"
                >
                    {{ trans('admin.master_data.user.action.reset') }}
                </Button>
                <Button
                    :as="Link"
                    variant="outline"
                    :href="index()"
                    :disabled="form.processing"
                    class="w-full sm:w-auto"
                >
                    {{ trans('admin.master_data.branch.action.back') }}
                </Button>
            </div>
        </form>
    </PageWrapper>
</template>
