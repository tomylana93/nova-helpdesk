...
- Routes are split in `routes/web.php`, `routes/settings.php`, and `routes/admin.php`, and wired from `bootstrap/app.php`.
...
- Action pattern: `app/Actions/{Domain}` with public `handle(...)`; existing domains include `Fortify`, `Settings`, `MasterData`, and `Uploads`.
